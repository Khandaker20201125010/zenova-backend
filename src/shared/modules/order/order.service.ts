...
export class OrderService {
  async createOrder(userId: string, data: CreateOrderInput): Promise<ApiResponse> {
    try {
  ...
  async getOrders(params: OrderQueryParams, userId?: string): Promise<ApiResponse> {
    try {
      const {
        page = 1,
        limit = 10,
  ...
  async getOrderByNumber(orderNumber: string, userId?: string): Promise<ApiResponse> {
    try {
  ...
  async updateOrderStatus(id: string, data: UpdateOrderStatusInput): Promise<ApiResponse> {
    try {
      const order = await prisma.order.findUnique({
        where: { id },
      });
        ...
        line_items: order.orderItems.map((item:any) => ({
          price_data: {
            ...
            product_data: {
              ...
              images: item.product.images ? (item.product.images as string[]).slice(0, 1) : [],
            },
  ...
  async handleStripeWebhook(payload: any, signature: string): Promise<ApiResponse> {
    try {
  ...
  async getUserOrders(userId: string, page: number = 1, limit: number = 10): Promise<ApiResponse> {
    try {
      const skip = (page - 1) * limit;
