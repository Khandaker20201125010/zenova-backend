...
export class NotificationController {
  async getUserNotifications(req: AuthRequest, res: Response): Promise<Response> {
    try {
      ...
      return res.status(StatusCodes.INTERNAL_SERVER_ERROR).json(
        errorResponse('Failed to get notifications', error.message)
      );
    }
  }
  ...
  async createNotification(req: AuthRequest, res: Response): Promise<Response> {
    try {
      ...
      return res.status(StatusCodes.INTERNAL_SERVER_ERROR).json(
        errorResponse('Failed to create notification', error.message)
      );
    }
  }

  async getNotificationStats(req: AuthRequest, res: Response): Promise<Response> {
    try {
      if (!req.user) {
        return res.status(StatusCodes.UNAUTHORIZED).json(
          errorResponse('Authentication required')
      ...
      return res.status(StatusCodes.INTERNAL_SERVER_ERROR).json(
        errorResponse('Failed to get notification stats', error.message)
      );
    }
  }
  ...
  async getUnreadCount(req: AuthRequest, res: Response): Promise<Response> {
    try {
      ...
      if (result.success && result.data) {
        ...
        return res.status(StatusCodes.OK).json(
          successResponse('Unread count retrieved', { unreadCount: 0 })
        );
      }
    } catch (error: any) {
      return res.status(StatusCodes.INTERNAL_SERVER_ERROR).json(
        errorResponse('Failed to get unread count', error.message)
      );
    }
  }
}