...
export class ProductService {
  ...
  async updateProduct(id: string, data: UpdateProductInput): Promise<ApiResponse> {
    try {
      const product = await prisma.product.findUnique({
        where: { id },
      });
  ...
  async uploadProductImages(productId: string, images: string[]): Promise<ApiResponse> {
    try {
      const product = await prisma.product.findUnique({
        where: { id: productId },
      });
  ...
  async removeProductImage(productId: string, imageUrl: string): Promise<ApiResponse> {
    try {
      const product = await prisma.product.findUnique({
        where: { id: productId },
      });
  ...
  async getRelatedProducts(productId: string, limit: number = 4): Promise<ApiResponse> {
    try {
      const product = await prisma.product.findUnique({
        where: { id: productId },
        select: {
  ...
  async toggleFavorite(userId: string, productId: string): Promise<ApiResponse> {
    try {
  ...
  async getUserFavorites(userId: string, page: number = 1, limit: number = 12): Promise<ApiResponse> {
    try {
      const skip = (page - 1) * limit;
