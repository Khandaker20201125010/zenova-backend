...
class UserService {
    ...
    async toggleTwoFactor(userId, data) {
        try {
            const user = await prisma_1.prisma.user.update({
                where: { id: userId },
            ...
            logger_1.default.error("Toggle 2FA error:", error);
            throw error;
        }
    }
    async uploadAvatar(userId, avatarUrl) {
        try {
            ...
            logger_1.default.error("Upload avatar error:", error);
            throw error;
        }
    }
    ...
    async getUserStats(userId) {
        try {
            const [ordersCount, totalSpent, reviewsCount, favoritesCount, activitiesCount,] = await Promise.all([
                ...
                prisma_1.prisma.activity.count({ where: { userId } }),
            ]);
            const stats = {
                orders: ordersCount,
    ...
    async getActivities(userId, page = 1, limit = 20) {
        try {
            ...
            logger_1.default.error("Get activities error:", error);
            throw error;
        }
    }
}
